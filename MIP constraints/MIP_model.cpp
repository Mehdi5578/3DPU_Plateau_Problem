...
Graph::Graph(int N, int M, int L) {
    for (int i = 0; i < N * M * L; ++i) {
        adjList[i]; // Create an empty adjacency list for each vertex
    }
...
void Graph::CreateGraph(int N,int M, int L){
    for (int i=0; i<N; i++){
        for (int j=0; j<M; j++){
            for (int k=0; k<L; k++){
                int index = mapping[Graph::getIndex(i,j,k,N,M,L)];
                if (i>0){
                    int index2 = mapping[Graph::getIndex(i-1,j,k,N,M,L)];
                ...
                if (j>0){
                    int index2 = mapping[Graph::getIndex(i,j-1,k,N,M,L)];
                    addEdge(index,index2);
                }
                if (j<M-1){
                    int index2 = mapping[Graph::getIndex(i,j+1,k,N,M,L)];
                ...
                if (k>0){
                    int index2 = mapping[Graph::getIndex(i,j,k-1,N,M,L)];
                    addEdge(index,index2);
                }
                if (k<L-1){
                    int index2 = mapping[Graph::getIndex(i,j,k+1,N,M,L)];
